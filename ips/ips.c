     ...
     )
{
    for (png_uint_32 y = 0; y < image->height; ++y) {
        // for (png_uint_32 i = 0; i < number_of_threads && ((y + i) < image->height); ++i) {
            ...
            task->output_image =
                image;
            task->row_index_to_process = y;
            task->last_row_index_to_process = y + 1;
...
void ips_sobel_filter(ips_task_t *task)
{
    png_uint_32 x, y;
    png_bytep source_pixel, destination_pixel;
    //printf("BRIGTNESS AND CONTRAST CALLED \n");
    int channel;
...
void *ips_thread_process_image_part(void *args)
{
    ...
    if pool->size == 0 {
        pthread_mutex_lock(&Resource_Counter_Mutex);
        pthread_cond_wait(&Got_Resources_Condition,
